...
class Sg2ImModel(nn.Module):
  ...
  def forward(self, objs, triples, obj_to_img=None,
    ...
    if masks_pred is None:
      ...
      layout, sampled = masks_to_layout(obj_vecs, layout_boxes, layout_masks,
                               obj_to_img, H, W)
    ...
    if self.layout_noise_dim > 0:
      ...
      layout_n = torch.cat([layout, layout_noise], dim=1)
    img = self.refinement_net(layout_n)
    return img, boxes_pred, masks_pred, rel_scores, sampled
